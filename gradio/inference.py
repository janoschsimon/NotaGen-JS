...
import torch
from utils import *
import config  # Verwende nun config für die Parameter
...
checkpoint = torch.load(config.INFERENCE_WEIGHTS_PATH, map_location=torch.device(device))
...
def inference_patch(period, composer, instrumentation):
    ...
    while True:
        ...
        prompt_patches = [[ord(c) for c in patch] + [patchilizer.special_token_id] * (config.PATCH_SIZE - len(patch)) for patch in prompt_patches]
        ...
        while True:
            ...
            if not tunebody_flag and patchilizer.decode([predicted_patch]).startswith('[r:'):  # start with [r:0/
                ...
                temp_input_patches = torch.cat([input_patches, r0_patch], axis=-1)
            ...
            if input_patches.shape[1] >= config.PATCH_LENGTH * config.PATCH_SIZE and not end_flag:
